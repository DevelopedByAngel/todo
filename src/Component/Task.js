...
const Task=(props)=>
{
	...
	date=date.slice(0,10)
	  return (
	      ...
	      <div className="taskdetails"><p taskid={id} id={"taskname"+id} className="taskname" >{task}</p><span id={"taskdue"+id}>{reverseDate(date)}</span><span onClick={(e)=>deletetask(id)}   className="delete"><img src={require('../assets/remove.png')} className="taskoptimg"/></span><span onClick={(e)=>updatetaskname(e,rerender)} id={id} className="edit"><img src={require('../assets/edit.png')} className="taskoptimg"/></span></div><p class="link"></p>
	      </div>
	    );
  }
const updatetaskname =(e,rerender)=>
{
	...
	$('.taskdueediting').attr('due',reverseDate($('#taskdue'+$(a).attr('id')).text()));
}
